...
int main(int argc, char const *argv[])
{
    ...
    gEpoll_.addFd(lfd, EPOLLIN | EPOLLET);
    ...
    while (1)
    {
        ...
        for (int i = 0; i < ready; ++i)
        {
            ...
            {
                // 新的连接到来  任务交给线程池处理
                pool.addTask(bind(acceptNewClient, lfd));
            }
...
void acceptNewClient(int lfd)
{
    // cout << "acceptNewClient 线程id:" << this_thread::get_id() << endl;
    ...
    // 将新的连接添加到红黑树中
    int ret = gEpoll_.addFd(cfd, EPOLLIN | EPOLLET);
    assert(ret);
}
